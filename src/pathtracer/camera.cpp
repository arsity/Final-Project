 ...
 */
    ...
    /**
     * This function convert degrees to radians
 ...
 */
    Ray Camera::generate_ray(double x, double y,double wavelength) const {
        ...
        auto right = tan(degrees_to_radians(hFov / 2));
        auto left = -right;
        auto top = tan(degrees_to_radians(vFov / 2));
        auto bottom = -top;

        auto u = x * (right - left) + left;
        auto v = y * (top - bottom) + bottom;

        auto dir = Vector3D(u, v, -1);
